...
var langAnswer = 'vi'
try {
  langAnswer = window.themeDisplay.getLanguageId().substring(0, 2)
} catch (error) {
  // langAnswer = prompt('Choice lange (dedault VI)')
  console.log(error)
}
...
class MultiLanguage {
  ...
  init (_languages, _vue) {
    ...
    if (languageKeys.includes(langAnswer)) { /** localStorage.getItem('vue-lang') */
      this.userLang = langAnswer /** localStorage.getItem('vue-lang') */
    }
    langAnswer = this.userLang /** window.localStorage.setItem('vue-lang', this.userLang) */
  }
  ...
  relaunchByDirective (el, binding, vnode) {
    ...
    if (!vnode.context.$options.messages ||
      !(current = getLanguage(vnode.context.$options.messages[ vnode.context.language ]))
    ) {
...
MultiLanguage.install = function (Vue, languages) {
  ...
  Vue.mixin({
    data () {
      return {
        language: langAnswer /** window.localStorage.getItem('vue-lang') */
      }
    },
    watch: {
      language (value) {
        multi.defaultLanguage = value
        langAnswer = value /** window.localStorage.setItem('vue-lang', value) */
    ...
    methods: {
      translate (path, params = {}) {
        ...
        if (!result) {
          result = multi.search(multi.languages[langAnswer], path, params)
        }
        return result
      }
    }
