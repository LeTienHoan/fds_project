...
import router from './router/newsBoard'
import { storeNewsBoard } from './store/newsBoard'
...
Vue.mixin({
  data () {
    return {
      get newsBoardOthersVariablePublicSuchAs () {
        return window.themeDisplay ? window.themeDisplay.getCDNBaseURL() + '/o/v2/mobilink/' : 'http://localhost:8081/api/'
      },
      get newsBoardClassName () {
        return window.ACTIVITY_CONST ? window.ACTIVITY_CONST.newsBoardClassName : 'org.mobilink.docmgt.model.NewsBoard'
      }
    }
  },
  methods: {
    newsBoardOthersMethodPublicSuchAs (value) {
      let priorityKey = ''
...
window.axios.get('/o/v2/mobilink/systemconfigs?start=-1&end=-1&type=config')
  .then(function (response2) {
    ...
    new Vue({
      el: '#new-board-mgt',
      router: router,
      store: storeNewsBoard,
      components: { NewsBoard },
      template: '<NewsBoard/>'
    })
  })
