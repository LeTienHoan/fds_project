<template>
  ...
  <div class="cv-weeks">
    ...
    <div v-for="(day, dayIndex) in daysOfWeek(weekStart)"
      :key="`${dayIndex}-day`"
      :title="holidaysTitle[isoYearMonthDay(day)]"
      :class="[
      'cv-day',
      'dow' + day.getDay(),
      ...
      {
        /** holiday: holidayCheck(day),
        weekend: weekends.indexOf(day.getDay()) >= 0, */
    ...
    <template v-for="e in getWeekEvents(weekStart)">
      ...
      <div
        ...
        @mouseenter="eventMouseenter(e, $event)"
        @mouseleave="eventMouseleave(e, $event)"
...
export default {
  ...
  data: () => ({ currentDragEvent: null, holidaysTitle: {} }),

  created () {
    this.$nextTick(() => {
      let vm = this
      let css = ''
      for (let i in vm.weekends) {
        css += '.dow' + vm.weekends[i] + ' {background: #eefff4 !important;} '
      }
      for (let i in vm.holidays) {
        const itemDate = new Date(Number(vm.holidays[i].holidayDate))
        const itemStr = vm.isoYearMonthDay(itemDate)
        css += '.d' + itemStr + ' {background: #ffeeee !important;} '
        css += '.d' + itemStr + ':hover:after { content: "' + vm.holidays[i].description.substring(0, 60) + '...' + '"; position: absolute; display: inline-block; top: 10px; left: 10px; background-color: rgba(0,0,0,0.8); padding: 6px 12px; border-radius: 6px; color: white; max-width: calc(100% - 20px); } '
      }
      css += '.d' + vm.isoYearMonthDay(new Date()) + ' {background: #ffe !important;} '
  ...
  methods: {
    ...
    // ******************************
    // THANHNV add event mouse
    // ******************************
    eventMouseenter (event, jsEvent) {
      jsEvent.preventDefault()
      jsEvent.stopPropagation()
      this.$emit('eventMouseenter', event, jsEvent)
    },
    eventMouseleave (event, jsEvent) {
      jsEvent.preventDefault()
      jsEvent.stopPropagation()
      this.$emit('eventMouseleave', event, jsEvent)
    },
    ...
    onClickEvent (e, jsEvent) {
      this.$emit('click-event', e, jsEvent)
    },
    ...
    holidayCheck (d) {
      let isoYearMonthDay = this.isoYearMonthDay(d)
      let checker = this.holidays.find(item => this.isoYearMonthDay(new Date(Number(item.holidayDate))) === isoYearMonthDay)
      if (checker) {
        this.holidaysTitle[isoYearMonthDay] = checker.description
        return true
      } else {
        this.holidaysTitle[isoYearMonthDay] = ''
        return false
      }
    },
...
/**
.theme-default .cv-day.weekend {
	background-color: #eefff4;
}
.theme-default .cv-day.holiday {
	background-color: #ffeeee;
}
.theme-default .cv-day.holiday.today, .cv-day.weekend.today {
