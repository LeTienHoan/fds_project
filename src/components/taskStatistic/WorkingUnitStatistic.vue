<template>
  <div class="mobilink-working-unit-statistic fullwidth">
    ...
    <AdvancedSearch :active="showAdvanced" typeSelectMulti="workingUnit"
    ...
    <table v-else class="table table-default-override table-basic">
      <thead>
        <tr>
          <th rowspan="2">STT</th>
          <th rowspan="2">{{translate('don_vi')}}</th>
      ...
      <tbody>
        <tr v-for="(item, index) in statisticItems" :key="'working-unit-statistic' + index"
        >
    ...
    >
      ...
      <template slot="content">
        ...
        <UserStatisticDetail  :userStatistic="statisticSelected"/>
      </template>
    </dia-log>
  </div>
</template>
...
const COMPONENT_NAME = 'mobilink-working-unit-statistic'
export default {
  name: COMPONENT_NAME,
  props: {
    name: {
  ...
  computed: {
    ...
    statisticItems () {
      return this.$store.getters['activityStatistic/workingUnitActivityStatisticItems']
    },
    monthAndYear () {
      let vm = this
  ...
  mounted () {
    this.$nextTick(() => {
      ...
      this.getListStatisticWorkingUnit()
      console.log('ACTIVE WORKIG UNIT')
    })
  },
  methods: {
    pageSizeChange () {
    ...
    getListStatisticWorkingUnit (changePage) {
      ...
      this.$store.dispatch('activityStatistic/getWorkingUnitActivityStatisticItems', queries).then(response => {
    ...
    showDetail (type, item, index) {
      ...
      this.statisticSelected = {
        index: index,
        workingUnitName: item.workingUnitName,
        ...
        queries: Object.assign(vm.advancedSearchQuery, {
          objectId: Number(item.hosterId) || Number(item.hostingId) ||
            Number(item.managerId) || Number(item.leaderId) ||
      ...
      if (type === 'tong_so') {
        ...
        this.statisticSelected.title = `
          Danh sách tổng số công việc của đơn vị <b>${item.workingUnitName}</b> (${vm.statisticName.toLowerCase()})
        `
      }
      if (type === 'tao_moi') {
        this.statisticSelected.queries.statisticColumnName = 'newCount'
        this.statisticSelected.title = `
          Danh sách công việc đã tạo mới của đơn vị <b>${item.workingUnitName}</b> (${vm.statisticName.toLowerCase()})
        `
      }
      if (type === 'ton_truoc') {
        this.statisticSelected.queries.statisticColumnName = 'remainingCount'
        this.statisticSelected.title = `
          Danh sách công việc tồn trước của đơn vị <b>${item.workingUnitName}</b> (${vm.statisticName.toLowerCase()})
        `
      }
      if (type === 'chua_thuc_hien') {
        this.statisticSelected.queries.statisticColumnName = 'unfulfilledOvertimeCount,unfulfilledOntimeCount'
        this.statisticSelected.title = `
          Danh sách công việc chưa thực hiện của đơn vị <b>${item.workingUnitName}</b> (${vm.statisticName.toLowerCase()})
        `
      }
      if (type === 'dang_thuc_hien_tong_so') {
        ...
        this.statisticSelected.title = `
          Danh sách tổng số công việc đang thực hiện của đơn vị <b>${item.workingUnitName}</b> (${vm.statisticName.toLowerCase()})
        `
      }
      if (type === 'dang_thuc_hien_con_han') {
        this.statisticSelected.queries.statisticColumnName = 'processingOntimeCount'
        this.statisticSelected.title = `
          Danh sách công việc đang thực hiện còn hạn của đơn vị <b>${item.workingUnitName}</b> (${vm.statisticName.toLowerCase()})
        `
      }
      if (type === 'dang_thuc_hien_tre_han') {
        this.statisticSelected.queries.statisticColumnName = 'processingOvertimeCount'
        this.statisticSelected.title = `
          Danh sách công việc đang thực hiện trễ hạn của đơn vị <b>${item.workingUnitName}</b> (${vm.statisticName.toLowerCase()})
        `
      }
      if (type === 'da_hoan_thanh_tong_so') {
        this.statisticSelected.queries.statisticColumnName = 'finishOntimeCount,finishOvertimeCount'
        this.statisticSelected.title = `
          Danh sách tổng số công việc đã hoàn thành của đơn vị <b>${item.workingUnitName}</b> (${vm.statisticName.toLowerCase()})
        `
      }
      if (type === 'da_hoan_thanh_dung_han') {
        this.statisticSelected.queries.statisticColumnName = 'finishOntimeCount'
        this.statisticSelected.title = `
          Danh sách công việc đã hoàn thành đúng hạn của đơn vị <b>${item.workingUnitName}</b> (${vm.statisticName.toLowerCase()})
        `
      }
      if (type === 'da_hoan_thanh_qua_han') {
        this.statisticSelected.queries.statisticColumnName = 'finishOvertimeCount'
        this.statisticSelected.title = `
          Danh sách công việc đã hoàn thành quá hạn của đơn vị <b>${item.workingUnitName}</b> (${vm.statisticName.toLowerCase()})
        `
      }
