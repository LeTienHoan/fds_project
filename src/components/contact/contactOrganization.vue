              ...
              <table class="table table-bs">
                <tr :key="index" v-for="(item, index) in contactItems">
                  ...
                  <td class="col-xs-2 text-xs-right">
                    <!-- <v-btn icon class="ma-0 pull-right" @click.stop="removeActivity(item,index)"> -->
                    <v-icon @click.stop="editContact(item,index)" class="pointer">edit</v-icon>
                    <v-icon @click.stop="removeContact(item,index)" class="pointer">cancel</v-icon>
                    <!-- </v-btn> -->
                  </td>
                </tr>
              </table>
...
<script>
  const COMPONENT_NAME = 'contact-organization'
  export default {
    name: COMPONENT_NAME,
    props: {
    },
